...
| Llama / MLX | Moved (v0.48, #1749) | — | The MLX and llama.cpp fuzz factories moved to the [manifold-mlx](https://github.com/ManifoldKit/manifold-mlx) / [manifold-llama](https://github.com/ManifoldKit/manifold-llama) companion packages with the backends. `--backend llama` / `--backend mlx` (and `scripts/fuzz.sh --with-mlx`) now error with a pointer; run those campaigns from the companion repos. |
...
| `empty-output-after-work` | `silent-empty` | [#487](https://github.com/ManifoldKit/ManifoldKit/issues/487) — `OllamaBackend.extractToken` drops the `thinking` field for reasoning models, leaving the user with a long compute and an empty assistant bubble. The headline first-day discovery. |
| `tool-call-validity` | 5 (`malformed-json-args`, `schema-violation`, `id-reuse`, `orphan-result`, `toolchoice-violation`) | [#627](https://github.com/ManifoldKit/ManifoldKit/issues/627) — tool-call correctness is the most fragile piece of any real integration. Activates when `--tools` is set or `RunRecord.toolCalls` is non-empty; reuses `JSONSchemaValidator`. `id-reuse` and `orphan-result` ship `confirmed` (zero-FP-by-construction transcript invariants); `malformed-json-args`, `schema-violation`, `toolchoice-violation` ship `flaky` pending corpus calibration under [#488](https://github.com/ManifoldKit/ManifoldKit/issues/488). |
...
Until [#487](https://github.com/ManifoldKit/ManifoldKit/issues/487) is fixed, this fires reliably against any reasoning model on Ollama: the backend swallows the `thinking` stream, generation completes cleanly with no error, and the user sees a long compute followed by an empty bubble.
...
Rationale: [#487](https://github.com/ManifoldKit/ManifoldKit/issues/487) — OllamaBackend silently dropped `message.thinking` because `OllamaBackendTests` had zero fixtures with that field. This matrix audits which protocol fields every backend has a fixture for and which it does not, so each `no` cell becomes a tracked follow-up rather than a latent "surfaced-by-the-fuzzer" bug. Source: [#503](https://github.com/ManifoldKit/ManifoldKit/issues/503).
...
| Ollama | `message.thinking` field | no | — | [#487](https://github.com/ManifoldKit/ManifoldKit/issues/487) |
| Ollama | `done_reason` variants (`stop`/`length`/`load`/`unload`) | no | — | [#507](https://github.com/ManifoldKit/ManifoldKit/issues/507) |
| Ollama | `eval_count` / `eval_duration` usage stats | no | — | [#508](https://github.com/ManifoldKit/ManifoldKit/issues/508) |
| Ollama | NDJSON mid-line byte split | no | — | [#509](https://github.com/ManifoldKit/ManifoldKit/issues/509) |
| Ollama | `/api/generate` legacy endpoint shape | no | — | [#510](https://github.com/ManifoldKit/ManifoldKit/issues/510) |
| Ollama | SSE stream limits (size / event rate) | no | — | [#511](https://github.com/ManifoldKit/ManifoldKit/issues/511) |
| Ollama | `Retry-After` header non-numeric variants | partial[^1] | `OllamaBackendTests.swift:230` (`rateLimitError_429`) | [#512](https://github.com/ManifoldKit/ManifoldKit/issues/512) |
...
| MLX | custom / gpt-oss / gemma4 markers | no | — | [#513](https://github.com/ManifoldKit/ManifoldKit/issues/513) |
| MLX | `maxThinkingTokens` cap | no | — | [#514](https://github.com/ManifoldKit/ManifoldKit/issues/514) |
...
| MLX | stop_reason variants (`.maxTokens` / EOS / cancel) | no | — | [#515](https://github.com/ManifoldKit/ManifoldKit/issues/515) |
| MLX | chat-template detection (missing `tokenizer_config.json`) | no | — | [#516](https://github.com/ManifoldKit/ManifoldKit/issues/516) |
| MLX | tool-call deltas (`<tool_call>` tags) | no | — | [#517](https://github.com/ManifoldKit/ManifoldKit/issues/517) |
...
| Llama | GGUF chat-template variants (ChatML / Llama3 / Gemma / Gemma4 / Mistral) | no | — | [#518](https://github.com/ManifoldKit/ManifoldKit/issues/518) |
| Llama | EOS token variants (`</s>` / `<|eot_id|>` / multi-EOG Gemma) | no | — | [#519](https://github.com/ManifoldKit/ManifoldKit/issues/519) |
| Llama | `n_batch` boundary (N, N+1, contextSize±1) | no | — | [#520](https://github.com/ManifoldKit/ManifoldKit/issues/520) |
| Llama | `llama_decode` error paths (prompt / generation) | no | — | [#521](https://github.com/ManifoldKit/ManifoldKit/issues/521) |
| Llama | `stopGeneration` fired mid-`llama_decode` | no | — | [#522](https://github.com/ManifoldKit/ManifoldKit/issues/522) |
...
| Foundation | `GenerationOptions.topP/topK/seed/repeatPenalty` passthrough | no | — | [#523](https://github.com/ManifoldKit/ManifoldKit/issues/523) |
| Foundation | `SystemLanguageModel.Availability` variant handling | no | — | [#524](https://github.com/ManifoldKit/ManifoldKit/issues/524) |
| Foundation | cancellation timing mid-partial | no | — | [#525](https://github.com/ManifoldKit/ManifoldKit/issues/525) |
| Foundation | structured content-part types (non-monotonic diff) | no | — | [#526](https://github.com/ManifoldKit/ManifoldKit/issues/526) |
...
| Claude | `thinking` / `thinking_delta` content blocks | no | — | [#527](https://github.com/ManifoldKit/ManifoldKit/issues/527) |
| Claude | `tool_use` streaming (`input_json_delta`) | no | — | [#528](https://github.com/ManifoldKit/ManifoldKit/issues/528) |
| Claude | `citations_delta` handling | no | — | [#529](https://github.com/ManifoldKit/ManifoldKit/issues/529) |
| Claude | `stop_reason` variants (`refusal` / `max_tokens` / `stop_sequence`) | partial[^4] | `SSEPayloadReplayTests.swift:57` | [#530](https://github.com/ManifoldKit/ManifoldKit/issues/530) |
| Claude | rate-limit error body shape + `anthropic-ratelimit-*` headers | partial[^5] | `ClaudeBackendTests.swift` (no body, only status) | [#531](https://github.com/ManifoldKit/ManifoldKit/issues/531) |
| Claude | interleaved content blocks (text + tool_use + thinking) | no | — | [#532](https://github.com/ManifoldKit/ManifoldKit/issues/532) |
