...
| `ChatViewModel.sendMessage(_:)` (and every `respond` overload) | `SendMessageError` | Thrown directly for preconditions (`.noActiveSession`, `.noModelLoaded`, `.modelLoading` — a load is in flight, distinct from `.noModelLoaded` since #2222), for a content-free turn (`.empty`), or wrapping whatever `ConversationRuntime` surfaced (`.runtime(any Error)` — typically a `ConversationError`). | No |
...
`InferenceError`, `CloudBackendError`, and `FallbackExhaustedError` already conformed to `BackendError` before this audit. The other five (`RetryExhaustedError`, `StructuredOutputError`, `ConversationError`, `SendMessageError`, `ManifoldKitError`) gained the conformance as part of this pass, each with a documented `isRetryable` reasoning on its own doc comment. Three of them (`ConversationError`'s four wrapping cases, `SendMessageError.runtime`, `RetryExhaustedError.lastError`) *defer* to a boxed underlying error's own `isRetryable` when it also conforms to `BackendError`, rather than guessing.

`AnyLanguageModelBridgeError` also gained the conformance in this pass, but its type — `ManifoldAnyLanguageModel` — was retired outright in #2435 (zero adoption); it is no longer part of the escapable set this article documents.
