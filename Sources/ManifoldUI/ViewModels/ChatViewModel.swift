...
public final class ChatViewModel {
    ...
    /// modules (`ManifoldUIModelManagement`) used to widen this to `package`
    ...
    ///
    /// `ManifoldUI` deliberately does not import `AppIntents` — the handler
    /// protocol defined in `ManifoldCore` keeps the seam framework-free.
    ...
    /// assembling shared services through ``ManifoldBootstrap`` get the
    ...
    // `configure(imageRuntime:)` (typically through `ManifoldBootstrap`);
    ...
    @_spi(BackendInternals) public init(
        ...
        // Non-optional runtime: callers assembling through `ManifoldBootstrap`
