...
- **Apple Foundation Models is an instant "Tier 0" — capability exists, not yet wired.** Wrap candidates as ``ModelSelectionCandidate`` and an OS-resident model (Apple FM, a zero-download `ModelSelectionProfile`) ranks head-to-head against downloadable upgrades in one list via `ModelFitScorer.rank(candidates:useCase:device:foundationAvailable:)`. When the provider is unavailable (`foundationAvailable: false`) it collapses to the bottom exactly like a model that won't load — no separate code path. **As of the 2026-07-03 inert-surface audit, no production call site constructs a `.resident` candidate** — `ManifoldKit.QuickStartSeed` maps every seed to `.downloadable(...)`, so this ranking never runs in the shipped app today. A host that wants Tier-0 ranking must build the `ModelSelectionProfile` and wrap it in `.resident(...)` itself.
