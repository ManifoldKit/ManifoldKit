...
struct FuzzChatCLI {
    ...
    static func printUsage() {
        let lines = [
            ...
            "  MK_OVERNIGHT_CAPTURE_DIR=PATH  with --session-scripts, archive every",
            "                      capture as JSON, including clean runs. Filenames",
            "                      include the session UUID and never overwrite, so",
            "                      workers may share one archive directory.",
