# `manifold-tools` — end-to-end tool-calling harness

`manifold-tools` is a CLI harness that proves tool calling works end-to-end on a real backend without spinning up a SwiftUI app. It ships:
...
| `containsAll` | `"values": [String]` | Every entry in `values` is present, compared case-insensitively |
| `containsAny` | `"values": [String]` | At least one entry in `values` is present, compared case-insensitively (use for contracts a correct model can paraphrase) |
...
| `read_file` | `{path}` | Reads from the bundled `Fixtures/manifold-tools/` sandbox (`ToolFixtures.bundledRoot()`) by default. Path escapes (symlink or `..`) return `.permissionDenied`; missing files return `.notFound`. |
