...
|---------|----------------|--------------|
| next | [`MIGRATION-security-policy.md`](MIGRATION-security-policy.md) | `networkPolicy` / `customHostTrustPolicy` / `allowUnpinnedCredentialedHosts` instance-scoped off `ManifoldConfiguration.shared` (#2293); `nil` still reads the transitional global. |
