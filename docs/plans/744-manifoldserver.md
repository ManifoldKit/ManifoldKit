# Plan — `ManifoldServer` (#744)

OpenAI-compatible HTTP server target that exposes any `ManifoldInference.InferenceBackend` over `/v1/chat/completions`. Lets users point Cursor, Continue, and any OpenAI-SDK client at a local Mac running ManifoldKit.
...
> **Update (#949):** The original three-target shape (`ManifoldServerCore` library + `ManifoldServerBackends` library + `ManifoldServer` executable) has been collapsed into a single `ManifoldServer` executable target. All routing, adapters, trait-aware backend provider, and the CLI entry point now live under `Sources/ManifoldServer/`. The `ManifoldServerCore` and `ManifoldServerBackends` library products no longer exist. References to those module names in the sections below are historical — the `Server` trait still gates the entire surface and the dependency boundaries (`ManifoldInference` only, `ManifoldBackends` only inside the executable wiring) are unchanged.

## What we are building

A new opt-in SwiftPM target `ManifoldServer` (executable) plus `ManifoldServerCore` (library, `package`-visible, where the routing lives). Together they implement:
...
These must land **before** any source files exist under `Sources/ManifoldServer*` on disk. Discovering them at PR-A review time is the bad outcome.

1. **Extend `Tests/ManifoldInferenceTests/TrafficBoundaryAuditTest.swift` with an inbound-listener rule.** The audit today only enumerates outbound `URLSessionProvider`-style I/O. Hummingbird is the repo's first inbound networking surface. Either (a) extend the audit's allowlist with an explicit `ManifoldServerCore` scope and the reviewer sign-off the audit demands, or (b) add a sibling `InboundTrafficBoundaryAuditTest` that enforces "only `ManifoldServerCore` may bind a socket; only `ManifoldServer` (executable) may construct it." Either path is acceptable; both must precede PR-A.
2. **`MockInferenceBackend` enhancements** in `Sources/ManifoldTestSupport/`: ability to emit scripted `.usage(prompt:completion:)` events, controlled parallel tool-call lane interleaving (out-of-order `index`), throttle-then-resume semantics. `SlowMockBackend`, `ChaosBackend`, and `MidStreamErrorBackend` already exist (`Sources/ManifoldTestSupport/MidStreamErrorBackend.swift`, etc.) and are referenced as-is. Pulling these into PR-A scope so PR-B isn't bloated by test-support churn.

## Decisions

| # | Decision | Rationale |
|---|----------|-----------|
| 1 | **Module shape: library `ManifoldServerCore` + thin executable `ManifoldServer`.** | SwiftLM's 3,068-line `Server.swift` (with documented type-checker timeouts) is a warning. Library form lets routes be unit-tested via Hummingbird's `Application.test()` against `MockInferenceBackend` from `ManifoldTestSupport` — no socket, no flake. |
| 2 | **`ManifoldServerCore` is `package`-visible**, not `public`. | Premature `public` is a one-way door — it locks routing/middleware shapes we'll want to refactor between v1 and v2. The menubar-embedding ask (#807) is the trigger for promotion to `public`; promote then. |
...
| 4 | **CLI: `swift-argument-parser`.** | Apple-stack standard. Keeps CLI surface readable next to `manifold-tools` / `fuzz-chat`. |
...
| 6 | **`ManifoldServerCore` depends on `ManifoldInference` only**, not `ManifoldBackends`. | The executable wires `BackendRegistrar` and concrete backends. Preserves the same one-way edge CLAUDE.md enforces for UI; unblocks #807 menubar embedding without dragging Hummingbird through anyone's default graph. |
...
| 12 | **Usage estimation for local backends via `TokenizerProvider`.** | Cloud backends populate `.usage(prompt:completion:)` from the wire; local backends don't. Estimating from the tokenizer is honest enough — better than omitting the field, which breaks OpenAI client billing/observability paths. Source declared via `x-manifold-usage-source: tokenizer-estimate` response header. |
...
| 19 | **Hand-roll narrow Codable models for `ChatCompletionRequest` / `Chunk` / `Response`.** | No shippable Swift OpenAI SDK exists that would fit. The surface we use is small (~150 LOC). `OpenAIToolEncoding` (outbound, in `Sources/ManifoldBackends/`) uses `[String: Any]` and the `#if CloudSaaS` gate is the only thing making it un-shareable; extraction to a shared `ManifoldOpenAIWire` Codable module is a real refactor but cross-cuts `CloudBackend` and the server simultaneously. **Lift on #807** as a separately-revertible PR. |
...
| 21 | **Use `SSEStreamLimits` from `ManifoldInference` for response-side budget caps.** | Same defensive limits the parser enforces inbound apply outbound. One bad request shouldn't stream 50MB. Note: `SSEStreamLimits` does not enforce flow-control backpressure — Hummingbird's writer backpressure is the relevant mechanism for slow consumers (covered in test strategy). |

## Module structure

```
Sources/
  ManifoldServerCore/          # library — package-visible, routes/models/middleware, no socket
                                # depends on ManifoldInference + Hummingbird only
    Models/
  ...
  ManifoldServer/                # executable — wires ManifoldBackends + BackendRegistrar
                                  # depends on ManifoldServerCore + ManifoldBackends + ArgumentParser
...
`ManifoldServerCore` deps: `ManifoldInference`, `Hummingbird`. **No `ManifoldBackends` dep.**
`ManifoldServerCoreTests` deps: `ManifoldServerCore`, `ManifoldTestSupport` (for `MockInferenceBackend`, `MidStreamErrorBackend`, etc.), Hummingbird's `XCTHummingbird` test product.
`ManifoldServer` (executable) deps: `ManifoldServerCore`, `ManifoldBackends`, `swift-argument-parser`.

## Wire mapping

`GenerationEvent` → OpenAI SSE chunks. **Primary tool-call path is whole-call `.toolCall(_)`.** Among v1 backends, `MLXBackend` is the only one with `supportsToolCalling: true` (`LlamaBackend` and `FoundationBackend` both declare `supportsToolCalling: false`). MLX's `BackendCapabilities` literal at `Sources/ManifoldBackends/MLXBackend.swift:57` does not set `streamsToolCallArguments`, so it defaults to `false` and MLX emits whole calls. The streamed-delta path exists for Ollama / cloud (`OllamaBackend.swift:123`, `OpenAIBackend.swift:81`, `ClaudeBackend.swift:93`) and is exercised by the expansion path below — gated out of v1 by decision 7.
...
- **Crash visibility.** Uncaught errors in route handlers surface via `OSLog` with category `com.manifoldkit.server` so they're visible in Console.app and `log stream`. Stack traces only at `--log-level debug`.
...
- **Mid-stream errors:** `MidStreamErrorTests` uses `Sources/ManifoldTestSupport/MidStreamErrorBackend.swift` to throw at token N; assert clean SSE error frame and `stopGeneration()` invoked exactly once.
...
| 12 — usage estimation for local backends | `EventMapperTests.testLocalBackendEmitsTokenizerEstimate` + `x-manifold-usage-source` header assertion |
...
- **Primary:** `Tests/ManifoldServerCoreTests/Integration/WireCompatReplayTests.swift` — Swift byte-replay suite mirroring `Tests/ManifoldBackendsTests/CloudBackendSSETests.swift` (which validates outbound parsing). Same fixture format, opposite direction. SDK-version-independent, deterministic, runs in CI.
...
```bash
swift test --filter ManifoldServerCoreTests --disable-default-traits --traits Server
```
...
2. **Import-grep guard** mirroring the existing UI→ModelManagement guard at `.github/workflows/ci.yml:105-117`: no source file outside `Sources/ManifoldServer*` may `import Hummingbird`.
3. **400-LOC file ceiling** for files under `Sources/ManifoldServer*` — `find Sources/ManifoldServer* -name '*.swift' -exec wc -l {} \; | awk '$1 > 400 {exit 1}'`. Hard rule, not plan prose. The SwiftLM cautionary tale is what this prevents.
...
4. **Release 0.14.0.** Highlight section in CHANGELOG with a 6–8 line code snippet showing `swift run ManifoldServer --backend mlx --model …` → `from openai import OpenAI; client.chat.completions.create(...)`.
...
| Local-backend usage estimates diverge from cloud actuals enough to break observability tools | Document the source explicitly via `x-manifold-usage-source: tokenizer-estimate` response header; revisit in v2 with a real counting hook. |
...
2. ~~`ManifoldServerCore` public vs `package`~~ → **`package`**. Premature `public` is a one-way door; promote when the menubar embedding ask (#807) materializes.
...
`ManifoldServer` was put behind a new `Server` SwiftPM trait (off by default)
