...
| `ManifoldAgentInstructions` | `AGENTS.md` ambient-instruction filesystem discovery (macOS-only), extracted from the retired `ManifoldSkills` (#2434). | Zero-adopter by the external-consumer bar (no shipping app or companion pins it yet) — but not inert: `ManifoldKit`'s `ConversationRuntimeOptions.withAgentInstructions(currentDirectory:stoppingAt:)` gives it a real in-repo caller and an integration test exercises the full discover → merge → `.systemPreamble` path. Requires explicit `import ManifoldAgentInstructions`; it is not re-exported by the Tier-1 `ManifoldKit` umbrella. |
