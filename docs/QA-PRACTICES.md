...
A capability score is **withheld** when generation coverage falls below `EVAL_MIN_COVERAGE_PCT` (default 80%). Both upstream scorers count cases that were never generated rather than treating them as holes, and IFEval is the worse of the two: a missing response *vacuously satisfies* whole instruction classes (an empty string is trivially "all lowercase"), so a run that persisted 3 of 541 responses still rendered a readable `Strict Accuracy 19.2%` — 104 of those "passes" being responses that never existed. The role models are also size-capped, because the previous "largest chat model" rule selected a 17.4 GB model that exceeded the harness's per-case timeout on essentially every case. Tracked upstream as manifold-eval#59 and #60.
