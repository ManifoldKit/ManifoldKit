...
| `matrix` | reference tool-calling scenarios via `manifold-tools`, scored into `ConformanceRecord`s and rendered as `MATRIX.md` | Ollama, all installed chat tags |
| `eval` | manifold-eval capability scoring — BFCL (tool-calling), IFEval (instruction-following), MTEB-STS (embeddings) | live Ollama generations, scored offline |
| `collate` | folds the per-backend `ConformanceRecord` legs into one cross-runtime matrix through manifold-eval's comparability guard | Ollama records + a real MLX run on the same family |
| `evalmain` | builds and tests manifold-eval against **this core checkout** rather than its pinned release, on an isolated clone | none (fixture-driven) |

The **IFEval** capability score is **withheld** when generation coverage falls below `EVAL_MIN_COVERAGE_PCT` (default 80%). Both upstream scorers count cases that were never generated rather than treating them as holes, but only IFEval is gated here, because only IFEval fails *upward*: a missing response *vacuously satisfies* whole instruction classes (an empty string is trivially "all lowercase"), so a run that persisted 3 of 541 responses still rendered a readable `Strict Accuracy 19.2%` — 104 of those "passes" being responses that never existed. BFCL fails *downward* instead — un-generated categories count as misses, which understates rather than flatters, so its `Overall` is reported with a scoped figure alongside it rather than withheld. The role models are also size-capped, because the previous "largest chat model" rule selected a 17.4 GB model that exceeded the harness's per-case timeout on essentially every case. Tracked upstream as manifold-eval#59 (BFCL) and #60 (IFEval).

The `collate` lane is deliberately **2-leg, not 3**. `manifold-tools-llama` has no `--emit-records` path, so there is simply **no llama row in the matrix at all** — its absence is recorded in the lane summary line, not in the rendered table. (The renderer builds rows only from records that exist; nothing emits a `notMeasured` placeholder for a leg that never ran. Do not read the missing row as a measured zero.) `manifold-tools-mlx` reports `coreCommit: "unknown"`, so collate's comparability guard cannot verify that leg; the lane surfaces the collator's own `records span N ManifoldKit core commits` warning rather than letting the operator learn to ignore a structurally inert guard.

A further trap the lane now checks for: the two legs name the same weights differently — Ollama by tag (`mistral:7b-instruct-v0.3-q4_K_M`), MLX by snapshot directory (`Mistral-7B-Instruct-v0.3-4bit`) — and those normalize to different model keys, so `collate` exits 0 having rendered two unrelated rows and **no cross-runtime comparison at all**. The lane asserts the rendered file actually contains a cross-runtime section and records a no-op, with both model strings, when it does not. Both limits are tracked upstream in the companion repos.

The `evalmain` lane exists because manifold-eval has no **pre-release** detector against core `main`. Its `core-bump.yml` is reactive (it bumps the pin after a core tag ships, reliably and within minutes) and its `rot-guard.yml` is weekly against the *current pin* — neither builds against unreleased core, and eval has no `canary.yml`, which manifold-llama and manifold-mlx both do. Principle 9 requires known consumers to be built against a change before it ships, and the assurance harness that grades the family is the one consumer with no such check.

It runs on a `git clone` of the eval repo, never the operator's checkout: `swift package edit` mutates `Package.resolved`, and a lane killed mid-run by the watchdog would otherwise leave the real repo silently resolving to core `main` for every later run. Because the clone captures the checkout **as committed**, the lane reports how far behind `origin/main` that checkout is, and how many uncommitted paths were excluded — testing a stale consumer and reporting a clean pass would be the same failure this section's preflight exists to remove.
...
```
Hours, not seconds — it cold-builds three packages then runs real inference. Lanes run **sequentially** (MLX and llama.cpp contend for GPU/unified memory), each with a unique `TMPDIR` and **no `--parallel`** (matches both repos' process-global-state constraints).

Before any lane runs, a **preflight** block probes every precondition the lanes depend on: companion-repo presence (by locating each repo's `Package.swift` rather than trusting a hardcoded path convention), whether Ollama is reachable (starting the daemon itself and hard-failing if it won't come up within `OLLAMA_START_TIMEOUT`), which installed Ollama tag satisfies each eval role — tool/instruct/embed — via `/api/show` capability metadata instead of a hardcoded tag literal, and whether the eval corpora are on disk (fetching them if not). Preflight writes its own report, `./.local-integration-runs/<stamp>/PREFLIGHT.md`, before the model inventory or any lane output.

A lane that was requested but did no real work — repo absent, no installed model satisfies a role, a corpus is missing, a filter matched nothing, every case `XCTSkip`d — is now recorded as a distinct `SKIP-NO-WORK` outcome in the Lane summary, never folded into `pass`, and **the script exits non-zero if any requested lane no-op'd**. That asymmetry — a silent skip rendering identically to a pass, under an exit code that was always 0 either way — is exactly what let eight unmet preconditions on the dev machine go unnoticed on 2026-07-28: every affected lane skipped silently and the run still produced a clean-looking report. (An earlier version of this doc claimed the model inventory alone made a silently-skipping lane "visible, not mistaken for a pass" — that claim was false; the inventory was necessary but not sufficient, which is the exact gap preflight + `SKIP-NO-WORK` accounting now closes.)

The model inventory itself is now recursive: GGUF live nested as `Models/gguf/<Family>/<file>.gguf`, and the old flat glob reported every family `MISSING` while GGUF sat on disk one directory down.
...
2026](https://github.com/actions/runner-images/issues/13739).

**What remains uncovered.** A hosted macOS-26 runtime is not a provisioned
Apple Intelligence device. Tests that require the downloaded system model or
real generation still `XCTSkip` when `FoundationBackend.probeIsReady()` is
false. Those paths remain physical-host release evidence, alongside the local
integration sweep. The CI move proves minimum-runtime linkage and the
