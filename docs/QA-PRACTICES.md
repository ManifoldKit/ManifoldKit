...
The `evalmain` lane exists because manifold-eval pins ManifoldKit `exact:` and — unlike manifold-mlx and manifold-llama — has no `Canary (core main)` workflow. Principle 9 requires known consumers to be built against a change before it ships, and the assurance harness that grades the family was the one consumer never checked. It runs on a `git clone` of the eval repo, never the operator's checkout: `swift package edit` mutates `Package.resolved`, and a lane killed mid-run by the watchdog would otherwise leave the real repo silently resolving to core `main` for every later run.
