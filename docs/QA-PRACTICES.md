...
The `collate` lane is deliberately **2-leg, not 3**. `manifold-tools-llama` has no `--emit-records` path, so the llama leg renders as an explicit hole row carrying that reason — never as a measured zero. `manifold-tools-mlx` reports `coreCommit: "unknown"`, so collate's comparability guard cannot verify that leg, and the lane says so on every run rather than letting the operator learn to ignore a structurally inert guard. Both limits are tracked upstream in the companion repos.
