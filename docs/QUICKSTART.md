...
| `ChatViewModel.onFirstLaunch` | [`Sources/ManifoldUI/ViewModels/ChatViewModel.swift`](../Sources/ManifoldUI/ViewModels/ChatViewModel.swift) | One-shot first-launch hook — present onboarding, seed defaults. | `vm.onFirstLaunch = { showOnboarding = true }` |
| `ChatViewModel.onUpgradeHintTriggered` | [`Sources/ManifoldUI/ViewModels/ChatViewModel.swift`](../Sources/ManifoldUI/ViewModels/ChatViewModel.swift) | Fired when the runtime detects a context/capability upgrade is warranted — surface a paywall or model-upgrade nudge. | `vm.onUpgradeHintTriggered = { showUpgradeSheet = true }` |
