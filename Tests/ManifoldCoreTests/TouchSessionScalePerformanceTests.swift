...
///
/// > Note on placement: this file lives in `ManifoldCoreTests` (not
/// > `ManifoldRuntimeTests` per the plan brief) because the in-memory
/// > SwiftData stack requires `ManifoldPersistenceSwiftData`, and the
/// > `ManifoldRuntimeTests` target is intentionally constrained to
