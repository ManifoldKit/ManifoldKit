...
/// helper that diverged file-by-file across `Tests/ManifoldUITests/`.
///
/// Lives in the UI test target (not `ManifoldTestSupport`) because
/// `ManifoldTestSupport` deliberately does not depend on `ManifoldUI` — that
/// dependency would force the four non-UI test targets that import
/// `ManifoldTestSupport` (`ManifoldCoreTests`, `ManifoldInferenceTests`,
/// `ManifoldInferenceSwiftTestingTests`, `ManifoldTestSupportTests`) to also
/// pull in `ManifoldUI` transitively, slowing every CI run.
///
...
func makeTestChatViewModel(
    ...
    if let userDefaults {
        ...
        suiteName = "ManifoldKitTests-\(UUID().uuidString)"
