...
/// ## Why
///
/// Initiative I7 split `Sources/BaseChatBackends/` into five trait-gated
/// source targets (`BaseChatCloudCore`, `BaseChatMLX`, `BaseChatLlama`,
/// `BaseChatFoundation`, `BaseChatCloud`) plus a thin re-export umbrella
/// (`Sources/BaseChatBackendsUmbrella/` backing the legacy `BaseChatBackends`
...
/// `Sources/BaseChatBackendsUmbrella/` would silently undo the split. This
...
final class PackageTopologyAuditTest: XCTestCase {
    ...
    private var sourcesRoot: URL {
        ...
        // Tests/BaseChatBackendsTests/<this>.swift -> repo root
        return here
    ...
    func test_umbrellaTargetHostsRegistrarsNotBackends() {
        let fm = FileManager.default
        let umbrella = sourcesRoot.appendingPathComponent("BaseChatBackendsUmbrella")
        var isDir: ObjCBool = false
        guard fm.fileExists(atPath: umbrella.path, isDirectory: &isDir), isDir.boolValue else {
            XCTFail("Sources/BaseChatBackendsUmbrella/ missing — the umbrella shim was deleted or renamed")
            return
        }
        ...
        for name in [
            ...
            XCTAssertTrue(
                ...
                "Sources/BaseChatBackendsUmbrella/\(name) missing — umbrella glue regressed"
            )
        }
        ...
        for forbidden in [
            ...
            XCTAssertFalse(
                fm.fileExists(atPath: path.path),
                "Sources/BaseChatBackendsUmbrella/\(forbidden) reappeared — file should live in its family target, not the umbrella"
            )
        }
    }
