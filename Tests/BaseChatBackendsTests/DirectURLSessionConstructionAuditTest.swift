...
/// accessors that wrap it) is the only place in BaseChatKit that builds a
    ...
    func test_sourcesContainNoUnauthorisedURLSessionConstruction() throws {
        ...
        if !offenders.isEmpty {
            ...
            XCTFail("""
                ...
                Route the call through URLSessionProvider (BaseChatBackends) or URLSessionFactory
                (BaseChatInference) so the redirect guard is installed. If the file genuinely
                belongs at the seam, add its relative path to allowlistedRelativePaths in
                Tests/BaseChatBackendsTests/DirectURLSessionConstructionAuditTest.swift with
