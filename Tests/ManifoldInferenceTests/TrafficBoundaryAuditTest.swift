...
final class TrafficBoundaryAuditTest: XCTestCase {
    ...
    private static let networkIOAllowlist: Set<String> = [
        ...
        // ManifoldInference so ManifoldHuggingFace can hit the same seam
        // without depending on ManifoldBackends.
        ...
        "ManifoldTools/ReferenceTools/HttpGetFixtureTool.swift",

        // Fuzz scenario that exercises network-shaped retry behaviour.
        "ManifoldFuzz/Scenarios/ThinkingAcrossRetryScenario.swift",

        // Test infrastructure — DenyAll, Mock, hardware probes. Live in
        // ManifoldTestSupport so production code never picks them up by
        // import.
    ...
    private static let privacyAPIAllowlist: Set<String> = [
        ...
        "ManifoldUI/Internal/ClipboardWriter.swift:UIPasteboard.general.setItems([[UTType.plainText.identifier: text]], options: pasteboardOptions())",
    ]
                ...
                if Self.matches(Self.processSpawnPattern, in: trimmed) {
                    if !Self.processSpawnAllowlist.contains(relativePath) {
                        offenders.append(.init(
                            ...
                            fix: "Remove the subprocess. If this is a CLI harness like ManifoldFuzz, add the file to processSpawnAllowlist with justification."
                        ))
                    }
                    continue
                }
    ...
    func test_rule6_importGraphBoundary() throws {
        ...
        let rules: [(prefix: String, forbidden: [String], why: String)] = [
            ...
            ("ManifoldInference/",
             backendFamilyModules + ["ManifoldRuntime", "ManifoldPersistenceSwiftData"],
             "ManifoldInference is the lowest production layer (apart from ManifoldTestSupport) and must not depend upward."),
        ]
                ...
                for forbidden in rule.forbidden {
                    if trimmed == "import \(forbidden)" || trimmed.hasPrefix("import \(forbidden) ") {
                        offenders.append(.init(
                            ...
                            file: relativePath, line: idx + 1, text: trimmed,
                            why: rule.why,
                            fix: "Remove the import. If you need a type from \(forbidden), expose it from a lower-layer module (e.g. promote a protocol to ManifoldInference)."
                        ))
                    }
                }
            }
        ...
        for rule in rules {
            ...
            for forbidden in rule.forbidden where Self.manifestTarget(block.text, declaresDependencyOn: forbidden) {
                offenders.append(.init(
                    ...
                    why: rule.why,
                    fix: "Remove the \(forbidden) dependency from \(rule.target). Depend on ManifoldRuntime/ManifoldInference ports or closure-inject the sibling surface instead."
                ))
            }
        }
    ...
    func test_rule7_traitGateSanity() throws {
        ...
        // (Post-C2 the only surviving non-stub traits are Server and Macros.)
        for expected in ["Server", "Macros"] {
            XCTAssertTrue(
    ...
    func test_sabotage_rule6_detectsForbiddenImportsAndManifestDependencies() {
        ...
        let manifest = """
            ...
            """
        let block = Self.manifestTargetBlock(in: manifest, targetName: "ManifoldUI")
        XCTAssertNotNil(block)
        XCTAssertTrue(Self.manifestTarget(block?.text ?? "", declaresDependencyOn: "ManifoldPersistenceSwiftData"))
        XCTAssertFalse(Self.manifestTarget(block?.text ?? "", declaresDependencyOn: "ManifoldPersistenceSwiftDataExtras"),
