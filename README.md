...
| Gemini, xAI, Groq, Mistral, OpenRouter (OpenAI-compatible endpoint) | `ManifoldKit` umbrella (or `ManifoldCloudSaaS`), `APIProvider.custom` + `OpenAIBackend` | ManifoldKit (core) |
...
ManifoldKit previously **consumed** AnyLanguageModel as a backend via the `ManifoldAnyLanguageModel` product. That bridge was retired in #2435 — zero adoption, plus dependency coupling to a pre-1.0 external package — and the providers it named (Gemini, xAI, Groq, Mistral, OpenRouter) are reached the same way any OpenAI-compatible endpoint is: `APIProvider.custom` + the native `OpenAIBackend` pointed at the provider's base URL. That path has tool calling and structured output the bridge never advertised, plus the cert pinning, retry, and circuit-breaking every `ManifoldCloudCore`-backed family gets for free. See [docs/MIGRATION-anylanguagemodel-retired.md](docs/MIGRATION-anylanguagemodel-retired.md).
...
- Update SPM dependencies to `.package(url: "https://github.com/ManifoldKit/ManifoldKit.git", ...)` with `from: "0.75.0"` <!-- x-release-please-version -->
