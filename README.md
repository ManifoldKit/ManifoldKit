...
```text
.package(url: "https://github.com/ManifoldKit/ManifoldKit.git", from: "0.79.0"), // x-release-please-version
...
There are **no default traits** — plain `swift build` is the full core build. The surviving opt-in traits are `Server` and `Macros` (genuine build-cost levers: Hummingbird and swift-syntax respectively). `SystemAIProviderExtension` and `CoreAI` were retired because they had no targets or build effect. See [docs/MIGRATION-platform-floor-26.md](docs/MIGRATION-platform-floor-26.md) for the platform and trait migration, and [docs/QUICKSTART.md → Customizing backends](docs/QUICKSTART.md#customizing-backends) for install steps.
...
Most of the surface builds and runs down to the package floor (iOS 26 / macOS 26). APIs introduced after that floor retain their own availability boundaries.

| Module / symbol | Min iOS | Min macOS | Notes |
|---|---|---|---|
| Package floor (`ManifoldKit`, `ManifoldInference`, `ManifoldRuntime`, `ManifoldUI`, …) | 26 | 26 | The n-1 baseline everything below is measured against. |
| `ManifoldFoundation` / `FoundationBackend` | 26 | 26 | Re-exported by the `ManifoldKit` umbrella. Availability still depends on Apple Intelligence being enabled and provisioned. |
| `ManifoldAppIntents` — `AppIntentToolExecutor` (the ToolExecutor bridge) | 26 | 26 | Separate opt-in product (not re-exported by the umbrella). |
| `ManifoldAppIntents` — `AskManifoldIntent` / `AskManifoldHandler` | 26 | 26 | Same opt-in product as above. |
...
WWDC 2026 validated AnyLanguageModel's bet: Apple opened Foundation Models itself to third-party providers behind a near-identical shape (`LanguageModel` / `LanguageModelExecutor`, driving `LanguageModelSession`). That is real validation for the "one API over many models" idea, and ManifoldKit still owns the production layer around it: its own cloud backends, turn loop, persistence, tools, and UI on the iOS 26 / macOS 26 floor.
...
- Update SPM dependencies to `.package(url: "https://github.com/ManifoldKit/ManifoldKit.git", ...)` with `from: "0.79.0"` <!-- x-release-please-version -->
