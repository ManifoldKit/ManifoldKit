#!/usr/bin/env bash
# Cold-start conformance — tier 3: ManifoldUI ChatView composition.
#
...
# paths-filters ("scripts/cold-start-tier3-chatview.sh") keep working
# unchanged. See scripts/README.md for the full inventory and
# scripts/cold-start.sh's header for what tier 3 actually exercises.
set -euo pipefail
exec "$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)/cold-start.sh" --tier 3
