...
for arg in "$@"; do
    case "$arg" in
        ...
        -h|--help)
            ...
            echo "                  --model deepseek/deepseek-r1:free --request-timeout 90 --minutes 5"
            ...
            echo "  --request-timeout S bounds each request's HTTP idle window (default 90s);"
            echo "  without it a slow/throttled free model can hang ~300s (the shared session"
            echo "  default) per request. Detectors already flag >60s, so 90s loses no signal."
            echo ""
...
if [[ $CLOUD_BACKEND -eq 1 ]]; then
    ...
    echo "    • --request-timeout S bounds each request's HTTP idle window (default 90s)."
    echo "      Without it a slow/throttled free model hangs ~300s (the shared session"
    echo "      default) per request; detectors flag >60s, so 90s costs no signal."
    echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
else
    LLAMA_HIT="miss"
    MLX_HIT="miss"
