...
```swift
.package(url: "https://github.com/ManifoldKit/ManifoldKit.git", from: "0.74.0") // x-release-please-version
```
...
.package(
    ...
    from: "0.74.0", // x-release-please-version
...
Never trap a recoverable condition. This is a property, not a fixed list of banned identifiers: `fatalError`, `assertionFailure`, `precondition`, and `preconditionFailure` all crash the process — `precondition`/`preconditionFailure` in release builds too, not just `swift test` — so none of the four belong on a path where the trapped condition can plausibly occur at runtime with a viable fallback (throw, log + safe default, or a compile-time-enforced init parameter). Three conditions must **all** hold for a trap to be legitimate: (a) the value is fixed once at object construction, never mutated during the object's lifetime; (b) the value originates from code the programmer wrote and controls directly (a literal, a hardcoded call site) — not from data that can enter the process from outside the programmer's control (a config file, a defaults key, a runtime-filtered list, a value forwarded through public API that a separate host app calls with its own data); **and (c) a trap here crashes the developer's own process (a build, a test run, a CLI tool the developer is driving directly), not a shipped app running on someone else's device.** All three must hold, not just (a): a `hopCap`/`chunkSize`/backend-list value that's fixed at construction but fed from a host app's config or a live-filtered list is exactly as dangerous as a mutable runtime toggle — it crashes real, shipped apps on a bad config value, and (a)-only reasoning missed this repeatedly in practice until sharpened during review. (b) alone is not sufficient either — it looked that way until (c) separated it out: `ManifoldFuzz` is a published `.library()` product a companion package can construct programmatically with no human at a terminal, so its `RotatingFuzzFactory(children:blockSize:)` fails (b) exactly like `hopCap` does, yet it correctly stays a trap, because it fails (c) — `ManifoldFuzz` is developer tooling that never ships inside an app; a bad value crashes the operator's own fuzz process immediately with a clear message, not a stranger's device with no way to diagnose it. `URLSessionProvider.networkDisabled` fails (a) — it's a "belt-and-suspenders" runtime kill-switch that can flip at any point during process lifetime, and used to be implemented with `precondition`, so a caller could trip it by simply constructing a cloud backend, crashing the whole host app instead of failing one request. `RedirectGuardDelegate.hopCap`, `FallbackBackend`'s empty-chain check, and `DocumentChunker`'s chunkSize/overlap all fail (b) and (c) together — each is public API fed from host-app-controlled runtime data (config, a `filter { $0.isReady }`-built list, `RAGConfiguration` read by `ManifoldBootstrap`) reachable from a shipped app's own bootstrap path, even though each value is fixed once at construction. `TrappingConstructAuditTest` (in `ManifoldInferenceTests`) enforces this across all four constructs; `trapping_construct_allowlist.txt` next to it records the per-site reasoning (all three conditions, explicitly) for the constructs that stay. Use `Log.*` for the fallback path. Reserve trapping constructs for true programmer errors with no recovery path.
