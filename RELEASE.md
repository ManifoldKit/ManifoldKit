...
2. **Companion-canary gate — CI-enforced, hard-blocking, no override flag.**
   ...
   `scripts/companion-canary-check.sh` automatically once `version.txt` is
   ahead of the latest published tag, but the mode depends on which event is
   firing (freshness, not just "is a release happening", is why):
